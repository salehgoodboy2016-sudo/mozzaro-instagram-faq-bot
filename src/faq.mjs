...
export function classifyFaq(rawText) {
  ...
  if (hasAny(text, ['ساعات العمل', 'مواعيد العمل', 'متى الدوام', 'دوام', 'تفتح', 'يفتح', 'فاتحين', 'مفتوح', 'مفتوحه', 'تقفل', 'يغلق', 'تسكر', 'متى تفتح', 'متى تقفل', 'متى تسكر', 'الان مفتوح', 'الحين مفتوح'])) topics.push(TOPICS.hours);
