...
const ALIASES = Object.freeze({
  phone: ['phone', 'phone_number', 'phone number', 'mobile', 'رقم', 'الجوال', 'رقم الجوال'],
...
export async function parseCustomerImport({ filename, base64 }) {
  ...
  if (/\.csv$/i.test(safeName)) matrix = splitCsv(buffer.toString('utf8').replace(/^\uFEFF/, ''));
    ...
    const firstSheetRows = Array.isArray(workbookRows[0]?.data) ? workbookRows[0].data : workbookRows;
    matrix = firstSheetRows.map((row) => row.map((value) =>
  ...
  try {
    ...
    if (req.method === 'POST' && pathname === '/admin/campaigns/api/imports/preview') {
      ...
      json(res, 200, { filename: parsed.filename, fileSha256: parsed.fileSha256,
        totalRows: preview.totalRows, eligibleRows: preview.accepted.length,
        pendingRows: preview.pending.length, optedOutRows: preview.optedOut.length,
        importedRows: preview.accepted.length + preview.pending.length + preview.optedOut.length,
...
export function campaignDashboardHtml() {
  ...
  <div id="app"><section class="grid"><div class="card"><div>جهات مؤهلة</div><div id="eligible" class="metric">—</div></div><div class="card"><div>بانتظار الموافقة</div><div id="pending" class="metric">—</div></div><div class="card"><div>قائمة الحظر</div><div id="suppressed" class="metric">—</div></div><div class="card"><div>القوالب المعتمدة</div><div id="templatesCount" class="metric">—</div></div><div class="card"><div>الإرسال</div><div class="metric danger">معطّل</div></div></section>
  <section class="card"><h2>استيراد العملاء</h2><p>عمود الجوال مطلوب. السجلات التي لا تحتوي دليلاً مكتملًا على الموافقة التسويقية تُحفظ بانتظار الموافقة ولا تصبح مؤهلة للحملات. تُقبل CSV وXLSX فقط.</p><input id="file" type="file" accept=".csv,.xlsx"><button id="preview">مراجعة الملف</button><button id="commit" disabled>اعتماد الاستيراد</button><pre id="importResult"></pre></section>
  ...
  <section class="card"><h2>محادثات واتساب</h2><p>إعادة تفعيل جميع المحادثات المتوقفة حاليًا. لن تُرسل الرسائل القديمة المعلّقة، وستُطبق قواعد التحويل البشري مجددًا على الرسائل الجديدة.</p><button id="resumeAll">إعادة تفعيل كل المحادثات</button><p id="resumeAllResult"></p></section>
  ...
  q('#resumeAll').onclick=async()=>{if(!confirm('سيتم إنهاء حالات التحويل البشري الحالية لكل المحادثات ومسح الرسائل القديمة المعلّقة دون إرسالها. متابعة؟'))return;try{const d=await whatsappAdmin('resume-all-conversations',{method:'POST',body:JSON.stringify({confirmed:true,requestId:crypto.randomUUID()})});q('#resumeAllResult').textContent='تمت إعادة تفعيل '+d.resumed+' محادثة، ومسح '+d.clearedPending+' رسالة معلّقة.'}catch(e){q('#resumeAllResult').textContent=e.message}};
